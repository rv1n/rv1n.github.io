...
class Settings(Base):
    ...
    Attributes:
        ...
        hosting_expiration_date: Дата окончания аренды хостинга
    ...
    hosting_expiration_date = Column(DateTime, nullable=True)
    
    def __repr__(self):
        return f'<Settings price_logging_time={self.price_logging_hour:02d}:{self.price_logging_minute:02d}>'
    
    def to_dict(self):
        if isinstance(self.hosting_expiration_date, datetime):
            hosting_date = self.hosting_expiration_date.date().isoformat()
        elif isinstance(self.hosting_expiration_date, date):
            hosting_date = self.hosting_expiration_date.isoformat()
        else:
            hosting_date = None
        return {
            'id': self.id,
            ...
            'hosting_expiration_date': hosting_date,
        }
